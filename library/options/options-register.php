<?php
/**
 * Clean Yeti Theme Options Settings API
 *
 * This file implements the WordPress Settings API for the 
 * Options for the Clean Yeti Theme.
 * 
 * @package 	Clean Yeti
 ...
 * 
 * Register theme_cleanyeti_options array to hold
 * all Theme options.
 * 
...
register_setting( 
	// $option_group
	'theme_cleanyeti_options', 
	// $option_name
	'theme_cleanyeti_options', 
	// $sanitize_callback
	'cleanyeti_options_validate' 
);

register_setting( 'theme_cleanyeti_customize', 'theme_cleanyeti_customize' );
/**
 ...
 */
function cleanyeti_options_validate( $input ) {
	// This is the "whitelist": current settings
	$valid_input = cleanyeti_get_options();
	// Get the array of Theme settings, by Settings Page tab
	$settingsbytab = cleanyeti_get_settings_by_tab();
	...
	$submittab = cleanyeti_get_current_tab();	
	foreach ( $tabs as $tab ) {
	...
	foreach ( $tabsettings as $setting ) {
		...
		if ( 'submit' == $submittype ) {
			...
			else if ( 'image' == $optiondetails['type'] ) {
				$valid_input[$setting] = esc_url( $input[$setting] );
			}
...
foreach ( $cleanyeti_tabs as $tab ) {
	...
	foreach ( $tabsections as $section ) {
		...
		add_settings_section(
			// $sectionid
			'cleanyeti_' . $sectionname . '_section',
			// $title
			$sectiontitle,
			// $callback
			'cleanyeti_sections_callback',
			// $pageid
			'cleanyeti_' . $tabname . '_tab'
		);
	}
}
...
foreach ( $option_parameters as $option ) {
	...
	if ( 'custom' != $optiontype ) {
		add_settings_field(
			// $settingid
			'cleanyeti_setting_' . $optionname,
			// $title
			$optiontitle,
			// $callback
			'cleanyeti_setting_callback',
			// $pageid
			'cleanyeti_' . $optiontab . '_tab',
			// $sectionid
			'cleanyeti_' . $optionsection . '_section',
			// $args
			$option
		);
	} if ( 'custom' == $optiontype ) {
 ...
 */
function cleanyeti_setting_callback( $option ) {
	...
	$fieldname = 'theme_cleanyeti_options[' . $optionname . ']';
	
	// Output checkbox form field markup
	if ( 'checkbox' == $fieldtype ) {
		?>
		<input type="checkbox" name="<?php echo $fieldname; ?>" <?php checked( $cleanyeti_options[$optionname] ); ?> />
		<?php
	}
	// Output radio button form field markup
		...
		foreach ( $valid_options as $valid_option ) {
			?>
			<input type="radio" name="<?php echo $fieldname; ?>" <?php checked( $valid_option['name'] == $cleanyeti_options[$optionname] ); ?> value="<?php echo $valid_option['name']; ?>" />
			<span>
	...
	// Output select form field markup
		...
		$valid_options = $option['valid_options'];
		?>
		<select id="<?php echo $optionname; ?>" name="<?php echo $fieldname; ?>">
		<?php 
		foreach ( $valid_options as $valid_option ) {
			?>
			<option <?php selected( $valid_option['name'] == $cleanyeti_options[$optionname] ); ?> value="<?php echo $valid_option['name']; ?>"><?php echo $valid_option['title']; ?></option>
			<?php
		}
		?>
		</select>
	...
	// Output text input form field markup
		...
		?>
		<input type="text" name="<?php echo $fieldname; ?>" value="<?php echo wp_filter_nohtml_kses( $cleanyeti_options[$optionname] ); ?>" />
		<?php
	}
    // Ouput Color picker
	else if ( 'color-picker' == $fieldtype ) {
		?>
		<input type="text" class="cyb-color-field" name="<?php echo $fieldname; ?>" value="<?php echo wp_filter_nohtml_kses( $cleanyeti_options[$optionname] ); ?>" data-default-color="<?php echo $option['default']; ?>"/>
		<?php
	}
	// Output Image Uploader
	else if ( 'image' == $fieldtype ) {
        ?>
        <input type="text" id="<?php echo 'text_' . $optionname; ?>" name="<?php echo $fieldname; ?>" value="<?php echo esc_url( $cleanyeti_options[$optionname] ); ?>" />
        <input id="<?php echo $optionname; ?>" class="button" type="button" value="<?php _e( 'Upload Image', 'cleanyeti' ); ?>" />
        <?php
    }
