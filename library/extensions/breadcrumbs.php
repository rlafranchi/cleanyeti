 ...
 *
 */
function cleanyeti_custom_breadcrumbs() {

    $showOnHome = 0; // 1 - show breadcrumbs on the homepage, 0 - don't show
    $delimiter = '</li><li>'; // delimiter between crumbs
    $home = __( 'Home', 'cleanyeti'); // text for the 'Home' link
    $showCurrent = 1; // 1 - show current post/page title in breadcrumbs, 0 - don't show
    $before = '<li class="current">'; // tag before the current crumb
    $after = '</li>'; // tag after the current crumb
    $archive = __( 'Archive by category', 'cleanyeti' );
    $search = __( 'Search results for', 'cleanyeti' );
    $tags = __( 'Posts tagged', 'cleanyeti' );
    $postedby = __( 'Articles posted by', 'cleanyeti' );
    $error404 = __( 'Error 404', 'cleanyeti' );
    ...
    if (is_home() || is_front_page()) {
        ...
        if ( get_query_var('paged') ) {
            ...
            echo __('Page', 'cleanyeti' ) . ' ' . get_query_var('paged');
...
add_action('cleanyeti_navigation_above', 'cleanyeti_custom_breadcrumbs', 2);
?>