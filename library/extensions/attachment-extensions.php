...
function cleanyeti_get_attachment_link( $id = 0, $size = 'thumbnail', $permalink = false, $icon = false, $text = false ) {
	...
	if ( empty( $_post ) || ( 'attachment' != $_post->post_type ) || ! $url = wp_get_attachment_url( $_post->ID ) )
		return __( 'Missing Attachment', 'cleanyeti' );
	...
	return apply_filters( 'cleanyeti_get_attachment_link', "<a href='#' title='$post_title' data-reveal-id='$id'>$link_text</a>", $id, $size, $permalink, $icon, $text );
}

function cleanyeti_attachment_gallery_data() {
    $post        = get_post();
    $id          = $post->ID;
    $id          = intval( $id );
...
function cleanyeti_image_attachments() {
    $post = get_post();
...
function  cleanyeti_get_domain($url) {
...
function cleanyeti_get_first_video() {
    ...
    if ( $domain == 'youtube.com' || $domain == 'vimeo.com' || $domain == 'hulu.com' ) {
        ...
        $content  = '<h3 class="subheader">' . __( 'Add a Video to The Post!', 'cleanyeti') . '</h3>' . "\n";
        $content .= '<p>' . __( 'Just paste the url of the video on a separate line to display the video.  Hulu, Youtube, Vimeo, and WordPress TV videos are currently supported.', 'cleanyeti' ) . '</p>' . "\n";
    }
    return $content;
}

function cleanyeti_insert_featured_image() {
	global $post;
...
add_filter('embed_oembed_html', 'cleanyeti_embed_oembed_html', 99, 4);
function cleanyeti_embed_oembed_html($html, $url, $attr, $post_id) {
