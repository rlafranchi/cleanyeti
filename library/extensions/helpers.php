 ...
 * @return $text
 */
function cleanyeti_trim_excerpt($text) {
	if ( '' == $text ) {
...
/**
 * cleanyeti_the_excerpt function.
 * 
 * @param string $deprecated (default: '')
 * @return $output
 */
function cleanyeti_the_excerpt( $deprecated = '' ) {
	global $post;
	$output = '';
	$output = strip_tags( $post->post_excerpt );
	$output = str_replace( '"', '\'', $output );
	if ( post_password_required($post) ) {
		$output = __( 'There is no excerpt because this is a protected post.', 'cleanyeti');
		return $output;
	}
...
/**
 * cleanyeti_excerpt_rss function.
 *
 * @return $output
 */
function cleanyeti_excerpt_rss() {
	global $post;
	$output = '';
	$output = strip_tags( $post->post_excerpt );
	if ( post_password_required( $post ) ) {
	...
	return apply_filters( 'cleanyeti_excerpt_rss', $output );

}

add_filter( 'cleanyeti_excerpt_rss', 'cleanyeti_trim_excerpt' );


/**
 * Create nice multi_tag_title
 */
function cleanyeti_tag_query() {
 ...
 *
 * @todo deprcate when cleanyeti_body_class becomes a filter of body_class
 * @return $term->name
 */
function cleanyeti_get_term_name() {
 ...
 * @return bool
 */
function cleanyeti_is_custom_post_type() {
	global $post; 
 ...
 * @return void
 */
function cleanyeti_numerical_link_pages( $args = array () )
{
    $defaults = array(
        'before'      => '<p>' . __( 'Pages:', 'cleanyeti' )
    ,   'after'       => '</p>'
    ,   'link_before' => ''
    ,   'link_after'  => ''
    ,   'pagelink'    => '%'
 ...
 */
function cleanyeti_locate_template_uri( $template_names, $load = false, $require_once = true ) {
	$located = '';
