 ...
 * by `Cyb_Scssc_parser` into a syntax tree, then it is compiled into another tree
 * representing the CSS structure by `Cyb_Scssc`. The CSS tree is fed into a
 * formatter, like `Cyb_Scssc_formatter` which then outputs CSS as a string.
 *
 * During the first compile, all values are *reduced*, which means that their
 * types are brought to the lowest form before being dump as strings. This
 * handles math equations, variable dereferences, and the like.
 *
 * The `parse` function of `Cyb_Scssc` is the entry point.
 *
 * In summary:
 *
 * The `Cyb_Scssc` class creates an instance of the parser, feeds it SCSS code,
 * then transforms the resulting tree to a CSS tree. This class also holds the
 * evaluation context, such as all available mixins and variables at any given
 * time.
 *
 * The `Cyb_Scssc_parser` class is only concerned with parsing its input.
 *
 * The `Cyb_Scssc_formatter` takes a CSS tree, and dumps it to a formatted string,
...
class Cyb_Scssc {
	...
	protected $formatter = "Cyb_Scss_Formatter_Nested";
	...
	public function compile($code, $name=null) {
		...
		$this->parser = new Cyb_Scss_Parser($name);
	...
	protected function importFile($path, $out) {
		...
		if (isset($this->importCache[$realPath])) {
			...
			$parser = new Cyb_Scss_Parser($path, false);
...
class Cyb_Scss_Parser {
	...
	static protected function makeOperatorStr($operators) {
		return '('.implode('|', array_map(array('Cyb_Scss_Parser','preg_quote'),
			$operators)).')';
	}
	 ...
	 *
	 * Consider the function Cyb_Scssc::keyword(). (All parse functions are
	 * structured the same.)
	 *
	 ...
	 *
	 * All of these parse functions are powered by Cyb_Scssc::match(), which behaves
	 ...
	 * avoid this, Cyb_Scssc::seek() is used to remember and set buffer positions.
	 *
 ...
 * @author Leaf Corcoran <leafot@gmail.com>
 */
class Cyb_Scss_Formatter_Nested extends Cyb_Scss_Formatter {
	public $close = " }";
 ...
 * @author Leaf Corcoran <leafot@gmail.com>
 */
class Cyb_Scss_Formatter_Compressed extends Cyb_Scss_Formatter {
	public $open = "{";
	public $tagSeparator = ",";
	public $assignSeparator = ":";
	public $break = "";
...
class Cyb_Scss_Server {
	...
	protected function compile($in, $out) {
		...
		if ( ! $wp_filesystem->put_contents( $out, $css, FS_CHMOD_FILE) ) {
			echo "error saving scss file!";
		}
		if ( ! $wp_filesystem->put_contents( $this->importsCacheName($out), serialize($this->scss->getParsedFiles()), FS_CHMOD_FILE) ) {
			echo "error saving scss file!";
		}
		return $css;
	}
	 ...
	 * @param \Cyb_Scssc|null $scss     SCSS compiler instance
	 */
